...
const ChatContainer = ({
  ...
  messages: Msg[];
  setSpanRef: ((id: string) => (el: HTMLSpanElement | null) => void) | any;
}) => {
  // const messages = [
  //   {
  //     id: 1,
  ...
  return (
    <ul className="p-4 text-white mx-auto w-full max-w-[var(--synkluna-query-chat-system-width)] pb-[var(--synkluna-chat-container-bottom-padding)] flex flex-col gap-[var(--synkluna-chat-container-message-gap)]">
      {messages.map((message) => (
        ...
        >
          <div className="group-data-[role=user]:bg-neutral-900 p-4 group-data-[role=user]:max-w-[var(--synkluna-chat-container-user-message-max-width)] group-data-[role=user]:rounded-[var(--synkluna-chat-container-user-message-radius)]">
            {/* <p>{message.content}</p> */}
            {message.role === "assistant" && message.text === "" ? (
              // ✅ unique ref per message id; no reuse
              <span ref={setSpanRef(message.id)} />
            ) : (
              message.text
            )}
          </div>
